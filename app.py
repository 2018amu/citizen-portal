...
collection = chroma_client.get_or_create_collection(name="citizen_services", embedding_function=embedding_function)
...
def admin_insights():
    ...
    for e in eng_col.find({}, {"job":1}):
        ...
        if isinstance(j,list): j=",".join(j) or "Unknown"
    ...
    for e in eng_col.find({}, {"service":1,"question_clicked":1,"desires":1}):
        ...
        for d in ds: desires[d]=desires.get(d,0)+1
    ...
    if data:
        ...
        premium_ml_candidates = premium_ml_candidates[premium_ml_candidates>1].reset_index()[['user_id','cluster']]
